...
function buildLayout(stubs) {
    ...
    from.returnTypes.forEach((rt, outIdx) => {
      for (const to of stubs) {
        ...
        if (inIdx >= 0) {
          ...
          edges.push({ from: from.source, to: to.source, outIdx, inIdx, messageClass: rt })
        }
      }
    })
  }
  ...
  for (const s of stubs) { if (!(s.source in col)) col[s.source] = 0 }
...
function buildSvgString(edges, positions, stubs, stubMap, flowMessages, flowExceptions, colOf, topOffset) {
  ...
  for (const e of edges) {
    ...
    if (!fp || !tp) continue
    const x1       = fp.x + NODE_W,  y1 = fp.y + outputPortY(stubMap[e.from], e.outIdx)
    const x2       = tp.x,            y2 = tp.y + inputPortY(stubMap[e.to],   e.inIdx)
...
const fmtJson = (obj, max = 30) => { const s = JSON.stringify(obj); return s.length > max ? s.slice(0, max) + '…' : s }
...
export class FcFlowGraph extends BaseElement {
  ...
  _openModal(stubSource, messageClass, msgData) {
    this._modalMsg = {
      stubSource,
      messageClass,
      payload: msgData?.message != null
        ? JSON.stringify(msgData.message, null, 2)
        : '{}',
      valid: true,
    }
            ...
            return html`
              <div class="fc-node"
                   ...
                   style="
                     ...
                     box-shadow:${selected
                       ? `0 0 0 2px ${st.color}44, 0 4px 20px ${st.color}22`
                       : '0 2px 8px rgba(0,0,0,0.4)'};
                   ">

                <!-- Header -->
                <div style="height:${HEADER_H}px; display:flex; align-items:center; gap:8px;
                            padding:0 10px; border-bottom:1px solid ${st.color}33;">
                  <span style="font-size:15px; color:${st.color}; flex-shrink:0;">${st.label}</span>
                  <span style="font-weight:700; font-size:13px; color:#f3f4f6; flex:1;
                               overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"
                        title="${stub.source}">${short(stub.source)}</span>
                  <span style="font-size:9px; color:#6b7280; text-transform:uppercase;
                               letter-spacing:.06em; flex-shrink:0;">${stub.messageEnum}</span>
                  ${excs.length ? html`
                    <span style="font-size:10px;color:#ef4444;font-weight:700;flex-shrink:0;">
                      ${excs.length}✕
                    </span>` : ''}
                </div>
                ...
                <div style="padding:${PORT_PAD_V}px 0;">
                  ${Array.from({ length: maxPorts }, (_, i) => {
                    ...
                    return html`
                      <div style="height:${slotH}px; display:grid; grid-template-columns:1fr 1fr;
                        ...
                        <div style="display:flex;flex-direction:column;justify-content:center;
                                    ...
                                    height:100%;">
                          ${inMsg ? html`
                            <span style="font-size:11px;color:${inColor};font-weight:600;
                                         font-family:monospace;white-space:nowrap;
                                         overflow:hidden;text-overflow:ellipsis;"
                                  title="${inMsg}">${short(inMsg)}</span>
                        ...
                        <div style="display:flex;flex-direction:column;justify-content:center;
                          ...
                          ${i === 0 && excs.length > 0 ? html`
                            <span style="font-size:11px;color:#ef4444;font-weight:600;
                                         font-family:monospace;white-space:nowrap;
                                         overflow:hidden;text-overflow:ellipsis;"
                                  title="${excs[0].message}">✕ Exception</span>
                            <span style="font-size:10px;color:#ef4444;opacity:0.7;
                                         white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"
                                  title="${excs[0].message}">
                              ${excs[0].message.length > 28 ? excs[0].message.slice(0, 28) + '…' : excs[0].message}
                            </span>
                          ` : outRt ? html`
                            <span style="font-size:11px;color:#6b7280;font-weight:600;
                                         font-family:monospace;white-space:nowrap;
                                         overflow:hidden;text-overflow:ellipsis;"
                                  title="${outRt}">${short(outRt)}</span>
                            <span style="font-size:10px;color:#6b7280;font-family:monospace;
                                         white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"
                    ...
                    return html`
                      <div class="mb-3">
                        <div class="flex items-center gap-2 mb-1">
                          ...
                          <button class="btn btn-xs btn-ghost btn-square opacity-50 hover:opacity-100"
                              ...
                              <div class="rounded-lg bg-base-300 p-3 mb-1">
                                <div class="flex items-center gap-2 mb-2">
                                  <span class="badge badge-xs ${
                                    m.messageType === 'finish'  ? 'badge-success' :
                                    m.messageType === 'process' ? 'badge-info' : 'badge-warning'
                                  }">${m.messageType}</span>
                                  <span class="text-xs text-base-content/40">${fmtDate(m.time)}</span>
                                </div>
                                <pre class="text-xs font-mono text-base-content/80 whitespace-pre-wrap overflow-auto">${JSON.stringify(m.message, null, 2)}</pre>
                              </div>
                            `)}
                      </div>
                    `
                  ...
                  <div class="alert alert-error text-xs mb-2 p-3">
                    <div class="w-full">
                      ...
                      <details class="mt-2">
                        ...
                        <pre class="mt-1 text-xs overflow-auto whitespace-pre-wrap opacity-70 max-h-48">${ex.traceString}</pre>
                      </details>
                    </div>
                  </div>
                `)}
      ...
      <dialog id="fc-stub-input-modal" class="modal">
        ${this._modalMsg ? html`
          <div class="modal-box w-[95vw] max-w-[95vw] h-[90vh] max-h-[90vh] p-0 flex flex-col overflow-hidden">
              ...
              <div>
                ...
                <div class="flex items-center gap-3 mt-1 flex-wrap">
                  ...
                  <span class="text-xs text-base-content/40">
                    Stub: <span class="font-mono">${short(this._modalMsg.stubSource)}</span>
                  </span>
                </div>
              </div>
              <button class="btn btn-sm btn-ghost btn-square mt-0.5 flex-shrink-0"
            ...
            <!-- Status bar -->
            <div class="flex items-center gap-2 px-4 py-2 border-t border-base-300 flex-shrink-0
            ...
            <div class="flex items-center justify-between px-5 py-3 border-t border-base-300 flex-shrink-0">
              <span class="text-xs text-base-content/30">
                Senden wird in einem späteren Release implementiert.
              </span>
