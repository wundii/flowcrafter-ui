...
export class FcFlowDetail extends BaseElement {
  ...
  async _load() {
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('flow-loaded', {
  ...
  render() {
    return html`
      <div>
        <button class="btn btn-sm btn-ghost border border-base-content/30 hover:border-base-content/50 mb-4" @click=${this._onBack}>← Zurück zur Flow-Liste</button>

        ${this.loading
          ? html`<div class="flex justify-center py-16"><span class="loading loading-spinner loading-lg"></span></div>`
          : this.error
  ...
  _renderDetail() {
    ...
    return html`
      ...
      <h3 class="font-semibold mb-2 text-sm uppercase tracking-wide text-base-content/50">
        Flow Graph
        <span class="normal-case font-normal ml-1 text-base-content/30">— Stub anklicken für Details · Message-Input anklicken zum Editieren</span>
      </h3>
      <fc-flow-graph
        .flow=${f}
