...
export class FcLogin extends BaseElement {
  ...
  render() {
    if (this._mode === 'loading') return html`
      <div class="min-h-screen bg-base-100 flex items-center justify-center">
        <span class="loading loading-spinner loading-lg"></span>
      </div>
    `

    if (this._mode === 'error') return html`
      <div class="min-h-screen bg-base-100 flex items-center justify-center p-4">
        <div class="alert alert-error max-w-sm">
          <span>${this._error}</span>
          <button class="btn btn-sm btn-ghost" @click=${() => { this._mode = 'loading'; this.connectedCallback() }}>
            ↻ Retry
          </button>
        </div>
      </div>
          ...
          <div class="card bg-base-200 border border-base-300">
            <div class="card-body gap-4">
              ...
              <h2 class="card-title text-base">
                ${isSetup ? 'Passwort festlegen' : 'Anmelden'}
              </h2>
              ...
              <form @submit=${this._onSubmit} class="flex flex-col gap-3">
                ...
                <div class="form-control">
                  ...
                  <input
                    ...
                    @input=${e => { this._password = e.target.value; this._error = null }}
                ...
                ${isSetup ? html`
                  <div class="form-control">
                    ...
                    <input
                      ...
                      .value=${this._confirm}
                      @input=${e => { this._confirm = e.target.value; this._error = null }}
                      ?disabled=${this._loading}
                      required
                    />
                  </div>
                ...
                >
                  ${this._loading
                    ...
                    : isSetup ? 'Passwort speichern' : 'Anmelden'}
                </button>
              </form>
            </div>
          </div>
