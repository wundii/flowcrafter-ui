...
export class FcApp extends BaseElement {
  ...
  async _onChangePassword(e) {
    ...
    if (next.length < 6)     { this._pwModal = { ...this._pwModal, error: 'Min. 6 Zeichen.' };                   return }
    if (next !== confirm)    { this._pwModal = { ...this._pwModal, error: 'Passwörter stimmen nicht überein.' }; return }
    this._pwModal = { ...this._pwModal, loading: true, error: null }
    const res = await auth.changePassword(current, next)
    if (res.error) { this._pwModal = { ...this._pwModal, loading: false, error: res.error }; return }
    this._closePwModal()
    await auth.logout()
    this._authed = false
  }
  ...
  render() {
    if (!this._authed) {
      return html`
        <fc-login @authenticated=${this._onAuthenticated}></fc-login>
      `
    }
      ...
      <div class="min-h-screen bg-base-100">
        <div class="navbar bg-base-200 shadow-sm px-4">
          ...
          <div class="flex-none absolute left-1/2 -translate-x-1/2">
            <a href="https://github.com/wundii/flowcrafter"
               target="_blank"
            ...
            <div class="modal-box max-w-sm">
              ...
              <form @submit=${this._onChangePassword} class="flex flex-col gap-3">
                <div class="form-control">
                  ...
                  <input type="password" name="current" class="input input-bordered input-sm"
                         autocomplete="current-password"
                         ?disabled=${this._pwModal.loading} required />
                </div>
                <div class="form-control">
                  <label class="label py-1"><span class="label-text text-xs">Neues Passwort</span></label>
                  <input type="password" name="next" class="input input-bordered input-sm"
                         autocomplete="new-password"
                         ?disabled=${this._pwModal.loading} required />
                </div>
                <div class="form-control">
                  <label class="label py-1"><span class="label-text text-xs">Neues Passwort wiederholen</span></label>
                  <input type="password" name="confirm" class="input input-bordered input-sm"
                         autocomplete="new-password"
                         ?disabled=${this._pwModal.loading} required />
                </div>
                ${this._pwModal.error ? html`
                  <div class="alert alert-error py-2 px-3 text-xs"><span>${this._pwModal.error}</span></div>
                ` : ''}
                <div class="modal-action mt-0">
                  ...
                  <button type="submit" class="btn btn-primary btn-sm" ?disabled=${this._pwModal.loading}>
                    ${this._pwModal.loading ? html`<span class="loading loading-spinner loading-xs"></span>` : 'Speichern'}
                  </button>
                </div>
              </form>
            </div>
            <form method="dialog" class="modal-backdrop"><button @click=${this._closePwModal}>close</button></form>
          ` : ''}
        </dialog>

        <main class="p-4"
              @flow-selected=${this._onFlowSelected}
              @flow-loaded=${this._onFlowLoaded}>
          ${this._renderContent()}
        </main>
      </div>
    `
